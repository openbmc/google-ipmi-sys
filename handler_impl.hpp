...
class Handler : public HandlerInterface
{
  public:
    ...
    uint64_t accelOobRead(const std::string &name, uint64_t address, uint8_t num_bytes) const override;
    void accelOobWrite(const std::string &name, uint64_t address, uint8_t num_bytes, uint64_t data) const override;
