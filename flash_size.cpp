                        ...
                        size_t* dataLen, HandlerInterface* handler)
{
    ...
    int length = sizeof(struct GetFlashSizeReply) + flashSize.length();
    ...
    reply->flashSizeLength = flashSize.length();
    std::memcpy(reply->flashSize, flashSize.c_str(), flashSize.length());
