...
static int gbmcConverVersion(VersionReader& version_reader, Revision& rev)
{
    ...
    rev.major = major;
    rev.minor = minor;
    rev.points[0] = htole16(point);
    rev.points[1] = htole16(subpoint);
    return 0;
}
    ...
    ipmiAppGetGBMCDeviceId(::ipmi::Context::ptr)
{
    ...
    if (!devIdInitialized)
    {
        ...
        printf("gBMC Version %d.%d.%d.%d\n", rev.major, rev.minor,
               rev.points[0], rev.points[1]);

        std::cerr << "gBMC Version: " << rev.major << "." << rev.minor << "."
                  << "." << rev.points[0] << "." << rev.points[1] << std::endl;

        if (r >= 0)
        {
            rev.major = std::min(rev.major, static_cast<uint8_t>(0x7F));
            devId.fw[0] = 0x7F & rev.major;
            rev.minor = std::min(rev.minor, static_cast<uint8_t>(99));
            devId.fw[1] = rev.minor % 10 + (rev.minor / 10) * 16;
            std::memcpy(&devId.aux, rev.points, 4);
        }
        devId.ipmiVer = 2;
...
std::unordered_map<std::string, std::string> OsReleaseReader::ReadReleaseFile()
{
    ...
    while (os_release_file)
    {
        ...
        if (std::regex_match(line, key_value_match, key_value_re))
        {
            ...
            items.emplace(key_value_match[1], key_value_match[2]);
        }
    }
    return items;
}