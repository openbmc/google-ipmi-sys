                         ...
                         HandlerInterface* handler)
{
    ...
    return ::ipmi::responseSuccess(SysOEMCommands::SysAccelOobDevceCount,
                                   replyBuf);
}
                        ...
                        HandlerInterface* handler)
{
    ...
    return ::ipmi::responseSuccess(SysOEMCommands::SysAccelOobDevceName,
                                   replyBuf);
}
...
Resp accelOobRead(std::span<const uint8_t> data, HandlerInterface* handler)
{
    ...
    size_t min_size =
        ReadNameHeader(data.data(), data.size_bytes(), sizeof(Request), &name, &payload);
    if (min_size != 0)
    {
