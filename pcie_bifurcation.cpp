...
Resp pcieBifurcation(std::span<const uint8_t> data, HandlerInterface* handler)
{
    ...
    std::string_view enableBifurication = ENABLE_BIFURICATION_CONFIG;
    auto bifurcation =
        enableBifurication.empty()
            ? handler->pcieBifurcation(/*index=*/data[0])
            : handler->pcieBifurcation(/*index=*/data[0], enableBifurication);
