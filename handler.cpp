...
typedef std::vector<std::pair<sdbusplus::message::object_path,
                              NamedArrayOfAnyTypeMaps>>
                               ...
                               uint8_t num_bytes) const
{
    ...
    {
        log<level::ERR>(
            "Failed to call Read on com.google.custom_accel",
            entry("WHAT=%s", ex.what()),
                            ...
                            uint8_t num_bytes, uint64_t data) const
{
    ...
    {
        log<level::ERR>(
            "Failed to call Write on com.google.custom_accel",
            entry("WHAT=%s", ex.what()),
            entry("DBUS_SERVICE=%s", ACCEL_OOB_SERVICE),
            entry("DBUS_OBJECT=%s", object_name.c_str()),
            entry("DBUS_INTERFACE=%s", ACCEL_OOB_INTERFACE),
