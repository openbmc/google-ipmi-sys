...
std::int64_t Handler::getRxPackets(const std::string& name) const
{
    ...
    {
        stdplus::print(stderr, "Invalid or illegal name: '%s'\n", name.c_str());
    ...
    {
        stdplus::print(stderr, "Path: '%s' doesn't exist.\n", path.c_str());
        throw IpmiException(::ipmi::ccInvalidFieldRequest);
    }
...
std::string Handler::getMachineName()
{
    ...
    if (ifs.fail())
    {
        stdplus::print(stderr, "Failed to open: %s\n", path);
        throw IpmiException(::ipmi::ccUnspecifiedError);
    }
    ...
    while (true)
    {
        ...
        if (ifs.eof())
        {
            stdplus::print(stderr,
                           "Failed to find OPENBMC_TARGET_MACHINE: %s\n", path);
            throw IpmiException(::ipmi::ccInvalidCommand);
        }
        if (ifs.fail())
        {
            stdplus::print(stderr, "Failed to read: %s\n", path);
            throw IpmiException(::ipmi::ccUnspecifiedError);
        }
