...
static std::unordered_map<uint32_t, std::string> pcie_i2c_map;

static int read_file(std::string& file_name, std::string& file_content)
{
    std::error_code ec;
                         ...
                         size_t* dataLen)
{
    ...
    for (auto& i2c_dev : fs::directory_iterator(i2c_dev_dir_path))
    {
        ...
        if (i2c_dev_string_number != std::string::npos)
        {
            ...
            pcie_slot_full_path.append("/slot-name");
            ...
            pcie_i2c_map[i2c_bus_number] = pcie_slot_name;
        }
    }

    auto reply = reinterpret_cast<struct PcieSlotCountReply*>(&replyBuf[0]);
    ...
    (*dataLen) = sizeof(struct PcieSlotCountReply);
...
ipmi_ret_t PcieSlotI2cBusMapping(const uint8_t* reqBuf, uint8_t* replyBuf,
                                 size_t* dataLen)
{
    if ((*dataLen) < sizeof(struct PcieSlotI2cBusMappingRequest))
    {
        std::fprintf(stderr, "Invalid command length: %u\n",
                     static_cast<uint32_t>(*dataLen));
        return IPMI_CC_INVALID;
    ...
    const auto request =
        reinterpret_cast<const struct PcieSlotI2cBusMappingRequest*>(
            &reqBuf[0]);
    ...
    uint8_t i2c_bus_number = 0;
    std::string pcie_slot_name;
    uint8_t entry = 1;
    for (auto iter = pcie_i2c_map.begin(); iter != pcie_i2c_map.end(); ++iter)
    {
        if (entry == request->entry)
        {
            i2c_bus_number = iter->first;
            pcie_slot_name = pcie_i2c_map[i2c_bus_number];
            break;
        }
        else
        {
    ...
    if ((sizeof(struct PcieSlotI2cBusMappingReply) + pcie_slot_name.length() -
         1) > MAX_IPMI_BUFFER)
    {
