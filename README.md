...
which is not included as part of the length.

The length, in bytes, of characters in the string (not including the trailing
NULL character).

The maximum length for any name is 43 bytes (not counting the trailing NULL).
...
The register data is always returned in 8 bytes (uint64). If number of bytes is
less than 8, the LSBs are used and the MSBs will be 0s.
...
As with read, the register data must be 8 bytes (uint64). If number of bytes is
less than 8, the LSBs are used and the MSBs are ignored.
