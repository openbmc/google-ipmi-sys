...
std::optional<bool> BifurcationDynamic::needsUpdate(uint8_t bus)
{
    auto currentDevice = pcieResources.find(bus);

    if (currentDevice == pcieResources.end())
    {
        return std::nullopt;
    }

    // Do not override PCIe Device.
    // PCIe Devices can and will override PCIe slots.
    // This mean that there are no bifurcation required for the slot.
    return currentDevice != pcieResources.end() &&
           currentDevice->second.type == PCIe::Type::Slot;
}
...
void BifurcationDynamic::findPCIeDevices()
{
    ...
    try
    {
        ...
        auto getObjects = systemBus.new_method_call(
            kEntityManagerServiceName, "/xyz/openbmc_project/inventory",
            kObjectManagerServiceName, "GetManagedObjects");
    ...
    for (const auto& [_, obj] : entities)
    {
        ...
        std::optional<bool> shouldUpdate = needsUpdate(bus);
                                               ...
                                               uint8_t bus, Cache& cache)
{
    ...
    uint8_t channels = maybePCIeDevice->second.channels;
    ...
    if (emptyCount != 0 && emptyCount < channels)
    {
        ...
        for (auto& out : output)
        {
            if (out.empty())
            {
                uint8_t currentLane = lanes +
                                      static_cast<uint8_t>(extraLanes-- != 0);
