...
std::vector<uint8_t> BifurcationDynamic::parseDevices(
    ::ipmi::Context::ptr ctx, const std::string& path,
    std::unordered_map<std::string, std::vector<uint8_t>>& localCache)
{
    // auto findDevice = localCache.find(path);
    // if (findDevice != localCache.end())
    // {
    ...
    {
        localCache[path] =
            std::vector<uint8_t>{static_cast<uint8_t>(deviceLanes.value())};
    ...
    {
        localCache[path] = std::vector<uint8_t>();
        return localCache[path];
    }
    ...
    {
        std::vector<uint8_t> current = parseDevices(ctx, childPath, localCache);
