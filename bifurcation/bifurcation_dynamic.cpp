                                       ...
                                       std::string_view name) noexcept
{
    ...
    try
    {
        ...
        // TODO(wltu): Do not parse json everytime
        auto entityConfig = parseConfig(entityConfigFile);
        readings = entityConfig.value("add_in_card", empty);
    }
    ...
    {
        if (i2cBus(ctx, path).value_or(0) != *instance)
        {
            continue;
        }
