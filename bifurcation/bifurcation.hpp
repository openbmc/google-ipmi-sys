...
using Cache = std::shared_ptr<std::unordered_set<uint8_t>>;
...
class BifurcationInterface
{
  public:
     ...
     * @return the number of lane taken by the device. std::nullopt if not
     * supported.
     */
    virtual std::optional<uint8_t> getBifurcation(uint8_t bus) = 0;
};
...
class Bifurcation : public BifurcationInterface
{
  ...
  private:
    ...
    std::vector<uint8_t> walkMux(std::string_view basePath,
                                 std::string_view path, Cache cache);
};
