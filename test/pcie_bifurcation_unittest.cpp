...
TEST(PcieBifurcationCommandTest, InvalidRequest)
{
    ...
    HandlerMock hMock;
    EXPECT_EQ(::ipmi::responseReqDataLenInvalid(), pcieBifurcation(request, &hMock));
}

TEST(PcieBifurcationCommandTest, ValidRequest)
{
    std::vector<std::uint8_t> request = {5};
    std::vector<uint8_t> expectedOutput = {4, 8, 4};

    HandlerMock hMock;
    EXPECT_CALL(hMock, pcieBifurcation(5, _)).WillOnce(Return(expectedOutput));
...
TEST(PcieBifurcationCommandTest, ReplyExceddedMaxValue)
{
    ...
    HandlerMock hMock;
    EXPECT_CALL(hMock, pcieBifurcation(5, _)).WillOnce(Return(expectedOutput));
    EXPECT_EQ(::ipmi::responseInvalidCommand(), pcieBifurcation(request, &hMock));
}
