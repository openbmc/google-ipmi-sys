...
TEST(HandlerTest, PcieBifurcationStatic)
{
    ...
    {
        EXPECT_THAT(h.pcieBifurcation(nullptr, bus, false),
                    ContainerEq(output));
    }

    for (const auto& bus : invalidBus)
    {
        EXPECT_TRUE(h.pcieBifurcation(nullptr, bus, false).empty());
    }
    ...
    {
        auto bifurcation = h2.pcieBifurcation(nullptr, i, false);
...
TEST(HandlerTest, PcieBifurcationDynamic)
{
    /*
      ...
      slot-99 (16 lanes, 4 max channel) PE3

      slot-100 (16 lanes, 1 max channel) PE4
      device-110 (16 lanes) PE5
    */

    std::vector<std::string> baseAssociations = {
        "slot-10", "slot-20", "slot-80", "slot-99", "slot-100", "device-110",
    };
    ...
    // 2-2-4-1-3-4
    // 2-2-4-1-1-1-1-4
    auto bifurcation = h.pcieBifurcation(nullptr, 0, true);
    ...
    bifurcation = h.pcieBifurcation(nullptr, 1, true);
    EXPECT_THAT(bifurcation, ElementsAre(8, 2, 2, 4));

    // 16, Valid PCIe slot with one device taking all lanes.
    bifurcation = h.pcieBifurcation(nullptr, 2, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // Valid PCIe slot with no device
    bifurcation = h.pcieBifurcation(nullptr, 3, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // Invalid PCIe slot
    bifurcation = h.pcieBifurcation(nullptr, 4, true);
    EXPECT_TRUE(bifurcation.empty());

    // 16, PCIe Device.
    bifurcation = h.pcieBifurcation(nullptr, 5, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // Invalid PCIe slot
    bifurcation = h.pcieBifurcation(nullptr, 6, true);
