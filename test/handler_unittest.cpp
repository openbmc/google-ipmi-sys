...
TEST(HandlerTest, PcieBifurcationStatic)
{
    ...
    {
        EXPECT_THAT(h.pcieBifurcationByIndex(nullptr, bus, false),
                    ContainerEq(output));
    }

    for (const auto& bus : invalidBus)
    {
        EXPECT_TRUE(h.pcieBifurcationByIndex(nullptr, bus, false).empty());
    }
    ...
    {
        auto bifurcation = h2.pcieBifurcationByIndex(nullptr, i, false);
...
TEST(HandlerTest, PcieBifurcationDynamic)
{
    ...
    */
    ...
    HandlerMock hMock;
    BifurcationDynamicMock bifurcationHelper;
    ...
    // 2-2-4-1-3-4
    // 2-2-4-1-1-1-1-4
    auto bifurcation = h.pcieBifurcationByIndex(nullptr, 0, true);
    EXPECT_THAT(bifurcation, ElementsAre(2, 2, 4, 1, 1, 1, 1, 4));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE0", true);
    EXPECT_THAT(bifurcation, ElementsAre(2, 2, 4, 1, 1, 1, 1, 4));
    ...
    bifurcation = h.pcieBifurcationByIndex(nullptr, 1, true);
    EXPECT_THAT(bifurcation, ElementsAre(8, 2, 2, 4));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE1", true);
    EXPECT_THAT(bifurcation, ElementsAre(8, 2, 2, 4));

    // 16, Valid PCIe slot with one device taking all lanes.
    bifurcation = h.pcieBifurcationByIndex(nullptr, 2, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE2", true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // 16
    // 16
    // 8-8
    bifurcation = h.pcieBifurcationByIndex(nullptr, 3, true);
    EXPECT_THAT(bifurcation, ElementsAre(8, 8));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE3", true);
    EXPECT_THAT(bifurcation, ElementsAre(8, 8));

    // 16, Valid PCIe slot with multiple device but exceeded max lanes.
    bifurcation = h.pcieBifurcationByIndex(nullptr, 4, true);
    EXPECT_TRUE(bifurcation.empty());
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE4", true);
    EXPECT_TRUE(bifurcation.empty());

    // Valid PCIe slot with no device
    bifurcation = h.pcieBifurcationByIndex(nullptr, 5, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE5", true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // Invalid PCIe slot
    bifurcation = h.pcieBifurcationByIndex(nullptr, 6, true);
    EXPECT_TRUE(bifurcation.empty());
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE6", true);
    EXPECT_TRUE(bifurcation.empty());

    // 16, PCIe Device.
    bifurcation = h.pcieBifurcationByIndex(nullptr, 7, true);
    EXPECT_THAT(bifurcation, ElementsAre(16));
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE7", true);
    EXPECT_THAT(bifurcation, ElementsAre(16));

    // Invalid PCIe slot
    bifurcation = h.pcieBifurcationByIndex(nullptr, 8, true);
    EXPECT_TRUE(bifurcation.empty());
    bifurcation = h.pcieBifurcationByName(nullptr, "/PE8", true);
    EXPECT_TRUE(bifurcation.empty());
