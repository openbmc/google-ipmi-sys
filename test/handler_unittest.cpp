...
ExpectGetManagedObjects(StrictMock<sdbusplus::SdBusMock> &mock)
{
    ...
    EXPECT_CALL(mock, sd_bus_message_new_method_call(
        ...
        .InSequence(s)
        .WillOnce(DoAll(
            SetArgPointee<1>(method),
            Return(0)));

    EXPECT_CALL(mock, sd_bus_call(
            _,          // sd_bus *bus,
    ...
    EXPECT_CALL(mock, sd_bus_message_enter_container(msg,
    ...
    EXPECT_CALL(mock, sd_bus_message_read_basic(msg, SD_BUS_TYPE_OBJECT_PATH,
        ...
        .InSequence(s)
        .WillOnce(DoAll(
            AssignReadVal<const char*>(object_path),
            Return(1)));
    ...
    EXPECT_CALL(mock, sd_bus_message_read_basic(msg, SD_BUS_TYPE_STRING,
                                                NotNull()))
        .InSequence(s)
        .WillOnce(DoAll(
            AssignReadVal<const char*>(property_grpc),
            Return(1)));
    ...
    EXPECT_CALL(mock, sd_bus_message_read_basic(msg, SD_BUS_TYPE_STRING,
        ...
        .InSequence(s)
        .WillOnce(DoAll(
            AssignReadVal<const char*>(value_port),
            Return(1)));

    EXPECT_CALL(mock, sd_bus_message_verify_type(msg, SD_BUS_TYPE_VARIANT,
                                                 StrNe("u")))
        .Times(AnyNumber())
        .InSequence(s)
    ...
    EXPECT_CALL(mock, sd_bus_message_read_basic(msg, SD_BUS_TYPE_UINT32,
                                                NotNull()))
        .InSequence(s)
        .WillOnce(DoAll(
            AssignReadVal<uint32_t>(port),
            Return(0)));
...
::testing::Sequence ExpectRead(
    StrictMock<sdbusplus::SdBusMock> &mock,
    uint64_t address,
    uint8_t num_bytes,
    uint64_t data,
    int sd_bus_call_return_value,
    uint8_t num_bytes_returned = NUM_BYTES_RETURNED_EQUALS_NUM_BYTES)
{
    ...
    EXPECT_CALL(mock, sd_bus_message_new_method_call(
            ...
            StrEq("com.google.custom_accel.BAR"),
            StrEq("Read")))
        .InSequence(s)
        .WillOnce(DoAll(
            SetArgPointee<1>(method),
    ...
    if (sd_bus_call_return_value >= 0) {
        EXPECT_CALL(mock, sd_bus_message_enter_container(
                msg, SD_BUS_TYPE_ARRAY, StrEq(SD_BUS_TYPE_BYTE_STR)))
            .InSequence(s)
            .WillOnce(Return(1));
...
::testing::Sequence
ExpectWrite(StrictMock<sdbusplus::SdBusMock> &mock, uint64_t address,
           uint8_t num_bytes, uint64_t data, int sd_bus_call_return_value)
{
    ...
    EXPECT_CALL(mock, sd_bus_message_new_method_call(
        ...
        .InSequence(s)
        .WillOnce(DoAll(
            TraceDbus("sd_bus_message_new_method_call"),
            SetArgPointee<1>(method),
            Return(0)));
    ...
    EXPECT_CALL(mock, sd_bus_message_append_basic(
        ...
        .InSequence(s)
        .WillOnce(DoAll(
            TraceDbus("sd_bus_message_append_basic(address) -> 1"),
            Return(1)));

    EXPECT_CALL(mock, sd_bus_message_open_container(
            method, SD_BUS_TYPE_ARRAY, StrEq(SD_BUS_TYPE_BYTE_STR)))
        .InSequence(s)
        .WillOnce(DoAll(
            TraceDbus("sd_bus_message_open_container(a, y) -> 0"),
            Return(0)));
    ...
    for (auto i = 0; i < num_bytes; ++i) {
        ...
        EXPECT_CALL(mock, sd_bus_message_append_basic(
                ...
                MatcherCast<const void*>(SafeMatcherCast<const uint8_t*>(
                    Pointee(Eq(byte))))))
            .InSequence(s)
            .WillOnce(DoAll(
                TraceDbus2("sd_bus_message_append_basic"),
                Return(1)));
    }
    ...
    EXPECT_CALL(mock, sd_bus_message_close_container(method))
        .InSequence(s)
        .WillOnce(DoAll(
            TraceDbus("sd_bus_message_close_container() -> 0"),
            Return(0)));
