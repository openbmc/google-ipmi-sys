...
TEST(HandlerTest, PcieBifurcation)
{
    ...
    std::ofstream outputJson(testConfig);
    outputJson << "1\n";
    outputJson.flush();
    outputJson.close();

    std::unordered_map<uint8_t, std::vector<uint8_t>> expectedMapping = {
        {1, {1, 3}}, {3, {3, 6}}, {4, {3, 2, 1}}, {6, {8}}};
    std::vector<uint8_t> validBus = {1, 3, 4, 6};
    std::vector<uint8_t> invalidBus = {0, 2, 5, 7};

    for (const auto& bus : validBus)
    {
        auto bifurcation = h.pcieBifurcation(bus, testConfig, expectedMapping);
        ASSERT_EQ(bifurcation.size(), expectedMapping[bus].size());
        for (size_t i = 0; i < bifurcation.size(); ++i)
        {
            EXPECT_EQ(bifurcation[i], expectedMapping[bus][i]);
        }
    }
    ...
    outputJson = std::ofstream(testConfig);
    outputJson << "0\n";
    outputJson.flush();
    outputJson.close();
    for (uint8_t i = 0; i < 8; ++i)
    {
        auto bifurcation = h.pcieBifurcation(i, testConfig, expectedMapping);
        ASSERT_TRUE(bifurcation.empty());
    }
}
