...
class MockHostBootTimeHandler : public Handler
{
  ...
  protected:
    std::string dbusSetDuration(const std::string& name,
                                uint64_t duration_ms) const override
    {
        return setDurationResult;
    }

    uint64_t dbusNotify(uint8_t checkpointCode) const override
    {
        return notifyResult;
    }
...
{
    const std::string strNotImportant = "NA" uint64_t u64NotImportant = 0;
