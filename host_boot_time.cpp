...
{
    if (data.size() < sizeof(struct HostBootTimeSetDurationRequestHeader) ||
        data.size() <
            sizeof(struct HostBootTimeSetDurationRequestHeader) +
                data[offsetof(HostBootTimeSetDurationRequestHeader, length)])
    {
    ...
    const uint8_t nameLen =
        data[offsetof(HostBootTimeSetDurationRequestHeader, length)];
    const uint8_t nameOff =
        offsetof(HostBootTimeSetDurationRequestHeader, length) +
        sizeof(decltype(std::declval<HostBootTimeSetDurationRequestHeader>()
                            .length));
    const std::string name{data.data() + nameOff,
                           data.data() + nameOff + nameLen};
    ...
    memcpy(&ms,
           data.data() +
               offsetof(HostBootTimeSetDurationRequestHeader, durationMs),
           sizeof(ms));

    std::vector<uint8_t> reply(sizeof(struct HostBootTimeSetDurationReply));
    try
    {
        reply[0] = handler->hostBootTimeSetDuration(name, ms);
    }
                        ...
                        const HandlerInterface* handler)
{
    ...
    memcpy(reply.data(), &timestamp_ms, sizeof(struct HostBootTimeNotifyReply));
